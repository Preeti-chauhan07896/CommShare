...
// Helper: convert selected image file to data URL string
...
async function addItem(e) {
  ...
  let finalImageUrl = imageUrl.value.trim() || undefined;
  if (imageFile && imageFile.files && imageFile.files[0]) {
    try {
      finalImageUrl = await fileToDataUrl(imageFile.files[0]);
    } catch (err) {
      console.error('Image file read error:', err);
      alert('Failed to read selected image file. Please try again or use an image URL.');
      return;
    }
  }
