...
// Helper: convert selected avatar file to data URL string
...
async function saveProfile(e) {
  ...
  // Decide avatar value (file takes precedence over URL)
  let avatarUrl = editAvatar.value.trim();
  if (editAvatarFile && editAvatarFile.files && editAvatarFile.files[0]) {
    try {
      avatarUrl = await fileToDataUrl(editAvatarFile.files[0]);
    } catch (err) {
      console.error('Avatar file read error:', err);
      alert('Failed to read selected avatar image. Please try again or use an image URL.');
      return;
    }
  }
